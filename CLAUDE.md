...
- **`ai.ts`** — Anthropic SDK integration. Builds category-specific prompts (bugs vs. user stories), parses JSON responses with regex fallback. Returns structured `AIAnalysis` objects.
...
- **`hooks/useBugs.ts`** — Central state hook. Manages ticket list, analysis state, abort controllers, and auto-triggers analysis when a single ticket is fetched by ID.
- **`services/api.ts`** — Typed fetch wrapper for all backend calls.
- **`components/bug/`** — `BugList → BugCard → (BugDetails + AIAnalysis)` component tree. AIAnalysis renders the nested JSON analysis result.
